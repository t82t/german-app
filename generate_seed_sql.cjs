...
categoriesData.forEach((cat, index) => {
  sql += `-- Kategorie: ${cat.name}\n`;
  sql += `WITH cat${index} AS (INSERT INTO categories (name, user_id) VALUES ('${cat.name}', NULL) RETURNING id)\n`;
  sql += `INSERT INTO vocabulary (category_id, user_id, english, german, respelling) VALUES \n`;
  
  const values = cat.words.map(w => 
    `((SELECT id FROM cat${index}), NULL, '${w.en.replace(/'/g, "''")}', '${w.de.replace(/'/g, "''")}', '${w.phon.replace(/'/g, "''")}')`
  );
  
  sql += values.join(',\n') + ';\n\n';
});

fs.writeFileSync('D:\\Antigravity\\german-app\\seed.sql', sql);
console.log('seed.sql erfolgreich generiert.');
